...
`:!{terminal command}` allows to run terminal commands from within vim (e.g. `:!pwd` shows us the current working directory), this takes you out of VIM so you can see the results and then you can press `ENTER` to jump back into VIM.

You can also try `Ctrl+z` (which suspends our VIM process) which takes us back to the terminal to run whatever commands we like and then in the terminal just type `fg` to bring VIM back to the (f)ore (g)round.
