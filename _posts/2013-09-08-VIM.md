...
In the web development community there is nothing that causes more discussion (other than 'spaces vs tabs') like "so, what editor/ide do you use?".

My timeline of editors has gone something along the lines of…
...
By that I mean, moving from using a mouse to open up a file system and double clicking on a file to open it to using a short-cut on the keyboard which lets me do a 'fuzzy' search instead (a 'fuzzy' search basically means you type in a few characters of what you're looking for and the software returns a list of files it thinks you might be after).

The problem with all these editors is that there is no real longevity to them (and on a side note: how efficient are we really being?). 

For example, the current hotness (as of 2013) is to use Sublime Text (a nice editor with good keyboard based features but also allows users to interact with it using a mouse). But what happens when Sublime Text goes out of business, or gets bought out by a competitor and then the code base dumped? 

But what can we do about this?

Well, there is an editor that has been around for a very long time (it has quite literally stood the test of time) and is available everywhere (in most cases is installed by default). 
