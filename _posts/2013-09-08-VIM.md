...
It seems popular editors nowadays are becoming more minimalist (oxymoron?). The UI appears to be reducing in favour of keyboard short-cuts they keep developer productivity high.
...
Imagine you *do* know where a file lives, fuzzy file searching is still quicker than reaching for your mouse, moving over to the file system and clicking on folders until you find the file and then double click to open it.

A lot of editors have a feature like this nowadays (Sublime Text is one such popular example).

The problem is that most editors don't normally have a long shelf live (non that are worth using anyway).
...
Basically, rather than you (every day) opening up the terminal and manually creating the same set of panes and windows, opening files in tmux, we can store the details of this 'set-up' as one instance of a layout. We do this by creating a text file that holds the configuration details for your layout.
...
Now we have a layout instance set-up, to use it we just run `tmuxinator start project` (or in my case `tmuxinator start work`).
...
You don't have to use the range numbers, you can just use: `:s/search/replace/gc`.
